...
gcloud run deploy "$SERVICE" \
  ...
  --concurrency 20 \
  --max-instances 3 \
  --set-env-vars "GCP_PROJECT_ID=$PROJECT_ID,GCP_LOCATION=us-central1,GEMINI_MODEL=gemini-2.5-flash,GEMINI_FALLBACK_MODEL=gemini-2.5-flash,RETRY_MAX_ATTEMPTS=3,RETRY_BASE_DELAY_S=5.0,BQ_DATASET=banca_raw,GITHUB_REPO=davideconcu-accenture/Agent-GCP-ETL-Code,GITHUB_BRANCH=main,FIRESTORE_COLLECTION=etl_agent_runs,GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=us-central1" \
